...
See [terminal-evidence.md](terminal-evidence.md) for the conquest/player and winning-arrival evidence requirements, early stop behavior, and final recorded-frame preservation. `release_review_ready` also permits reviewed non-winning outcomes; a win requires an explicitly reviewed `victory_conquest` or `victory_space` outcome.

## Limits and release use

New finalizations additionally bind retained original-game and dashboard PNGs to an actual final-state sample in the ledger. Older recordings without this optional evidence retain their original verification behavior. The current recorder journal records counts and timings, **not hashes of the MP4, recording manifest or complete sample ledger**. The verifier computes and reports those hashes now but labels them as not anchored in the journal. Original dashboard PNG samples were streamed to the encoder and not retained, so their ledger hashes cannot be compared with original source pixels afterward. Encoded-video timing checks do not establish that every frame is visually correct.
